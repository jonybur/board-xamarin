...
namespace Solution
{
	public static class Preview
	{
		...
		private static UIImageView redBox;

		private static float Compression = .3f;
		private static float ThumbCompression = .1f;
		...
		public static void Initialize (UIImage image, CGPoint ContentOffset)
		{
			...
			UIImageView redBox = CreateRedBox (frame);

			uiView.AddSubviews(redBox, uiImageView);

			CheckCollision ();

		}

		public static async Task Initialize (TextBox textBox, CGPoint ContentOffset, Action refreshContent, UINavigationController navigationController)
		{
			...
			uiView.AddSubview (CreateRedBox (uiView.Frame));

			CheckCollision ();
		}

		private static UIImageView CreateRedBox(CGRect frame)
		{
			...
			redBox = new UIImageView(black);

			return redBox;
		}

		private static void SetRedBoxAlpha (float alpha)
		{
			redBox.Alpha = alpha;
		}
		...
		private static void CheckCollision()
		{
			...
			// 		 maybe save .frame in the database? check how to deal correctly with rotated views

			CGRect aux = new CGRect (uiView.Center.X, uiView.Center.Y, uiView.Frame.Width, uiView.Frame.Height);

			if (BoardInterface.CheckRectangleFCollision(aux))
			{
				Collision = true;
				SetRedBoxAlpha(.5f);
			}
		...
		public static Picture ConvertToPicture()
		{
			...
			string encodedThumb = CreateThumbnail (((UIImageView)uiView.Subviews [1]).Image);

			CGRect rec = new CGRect (uiView.Center, uiView.Bounds.Size);
			Picture p = new Picture (encodedPicture, encodedThumb, Rotation, rec, CloudController.BoardUser.Id);
